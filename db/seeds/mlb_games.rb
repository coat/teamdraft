...
# Loads a snapshot of the 2025 MLB postseason into the local DB so developers
...
module Seeds
  module MlbGames
    SEASON_YEAR = 2025
    DATA_PATH = Rails.root.join("db/seeds/mlb_2025_games.json")

    def self.call
      sport = Sport.find_by(key: "mlb")
      return warn_missing("MLB sport not installed; run sports:install[mlb] first.") unless sport
    ...
    def self.ensure_season(sport)
      sport.seasons.find_or_create_by!(year: SEASON_YEAR) do |s|
        s.label = "#{SEASON_YEAR} MLB Season"
        s.starts_on = Date.new(SEASON_YEAR, 3, 15)
        s.ends_on = Date.new(SEASON_YEAR, 11, 5)
        s.status = "completed"
      end
    end
    ...
    def self.load_games(season, season_team_ids)
      ...
      Rails.logger.info { "[seeds] MLB #{SEASON_YEAR} games: #{created} created, #{updated} updated, #{games.size} total." }
    end
